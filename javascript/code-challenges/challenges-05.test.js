...
const toLastNames = people => {
  let newNames = people.map(e => e.firstName + ' ' + e.lastName);
  return newNames;
};
...
const addValues = (arr) => {
  let newVal = arr.reduce(((acc, curVal) => acc + curVal), 0);
  return newVal;
};
...
const addPurchases = (arr) => {
  let total = arr.reduce(((acc, curVal) => acc + curVal.purchasePrice), 0);
  return total;
};
...
const countNumberOfElements = (arr) => {
  let totalLength = arr.reduce(((acc, curVal) => acc + 1));
  return totalLength;
};
...
const returnNames = (arr) => {
  let onlyNames = arr.reduce(((acc, curVal) => {
...
const reversedString = (str) => {
  let split = str.split('');
  let reverseOrder = split.reduce(((acc, curVal)=> curVal + acc));
